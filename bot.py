...
Как оставить заявку? Все просто:

1. Нажмите /start, если еще не нажимали.
...
5. Все ваши сообщения сразу приходят мне в личку. Никаких автоответов — все читаю и разбираю лично, вручную.
...
Я — настоящая, все делаю сама и очень стараюсь для каждого.
...
Когда все напишете — нажмите кнопку "Подтвердить"
...
RESPONSE_WAIT = """Спасибо, я все получила!
Ваша заявка ушла ко мне — как только подойду к ней, сразу начну работу.

Обычно отвечаю в течение 2 часов, но все зависит от загруженности.
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "tarot":
        ...
        if not data or not data["text"].strip():
            await query.message.reply_text("Вы еще ничего не написали.")
            return
...
if __name__ == "__main__":
    ...
    app.add_handler(MessageHandler(~filters.TEXT, ignore_media))
    app.run_polling()
